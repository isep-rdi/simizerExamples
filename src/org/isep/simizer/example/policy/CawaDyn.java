...
/**
 * This class implements the Cost AWare Algorithm for cost based request routing.
 * The algorithm records a fixed (REQUEST_THRESOLD) number of requests.
 * When the thresold is reached, the algorithmclassifies the requests based on their cost and
 * parameters in a number of classes equal to the number of nodes. 
 * This step is performed by a call to @link simizer.utils.Clustering class.
 * 
 * @author R. Chiky
 */
public class CawaDyn implements Policy, Policy.Callback {
    ...
    /**
     * Performs a Round Robin request distribution until the algorithm status is CLUSTERED.
     * Once clustering as been performed, each node is associated to a request prototype (which is the cluster center of each class)
     * When a request arrives it is compared to each prototype and routed to the closest one (Euclidean distance of the parameter string).
     * Please look at @link Vector and @link Request files.
     * @param r Request
     * @return target node
     */
    @Override
    ...
    /**
     * Method automatically called when a request is over and returned to the client.
     * Each request execution time and parameter is recorded by this method.
     * When the number of recorded request reaches REQUEST_THRESOLD, the clustering algorithm is launched.
     *  
     * @param n
     * @param r 
     */
    ...
    /**
     * This method runs threee algorithm steps:
       ...
       synchronized(this) {
        ...
        Clustering myClust=new Clustering(
            ...
            for(int i=0;i<costMatrix.length;i++) {
                for (int j=0;j<nodeList.size(); j++) System.out.print(costMatrix[i][j] + ";");
                System.out.print("\n");
            }
